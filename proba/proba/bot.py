...
bot = telebot.TeleBot('1422265418:AAFyfsfns65oxt4rGcZoZCUKCJT4fO1s84A')
...
def action_pick(message):
    if message.text == 'Расписание пар':
        ...
        literature_pick.add(english, math, dm, algebra_geometrie, it)
...
def literature(message):
    if message.text == 'Английский язык':
            book = open('/home/kristina/eng/Учебник для технических университетов и вузов Орловская И.В..pdf', 'rb')
            bot.send_document(message.chat.id, book)

    elif message.text == 'Математический анализ':
            book = open('/home/kristina/matan/Мат. анализ сборник задач Б.П.Демидович.pdf', 'rb')
            bot.send_document(message.chat.id, book)

    elif message.text == 'Алгебра и геометрия':
            book = open('/home/kristina/alg_geom/Бортаковский А.С., Пегачкова Е.А. Типовые задачи по аналитической геометрии.epub', 'rb')
            bot.send_document(message.chat.id, book)

    elif message.text == 'Дискретная математика':
            book = open("/home/kristina/dm/Алгебра множеств, бинарные отношения в примерах и задачах Нефёдов В.Н..epub", 'rb')
            bot.send_document(message.chat.id, book)

    elif message.text == 'Информатика':
            book = open("/home/kristina/it/Лекции Зайцева.epub", 'rb')
            bot.send_document(message.chat.id, book)
